...
### Paramétres ###
nom_fichier_corpus = 'input.txt'
alphabet_anglais=c("a",)
...
calcule_frequence_bigrames <- function(texte, alphabets) {
  ...
  valeur_minimal = unique(sort(matrice_bigrames))[2]*1e-2
...
calcule_frequence_trigrames <- function(texte, alphabets) {
  ...
  n = length(alphabets)
  for (compteur_anteprecendent in 1:n ) {
    lettre_anteprecedente = alphabets[compteur_anteprecendent] #commentaire
  ...
  valeur_minimal = unique(sort(matrice_trigrames))[2]*1e-2
...
permutation_des_lettres_par_unifome = function (alphabets) {
  ...
  indices_lettres = sample(length(alphabets), size = 2)

  lettre_1 = alphabets[indices_lettres[1]]
  lettre_2 = alphabets[indices_lettres[2]]
...
texte.voisin = function(texte,lettres_a_permuter){ #texte_voisin
  texte_decrypte = ''
  for(i in 1:str_length(texte)){
    if(str_sub(texte,i,i) == lettres_a_permuter[1]) {
        texte_decrypte = str_c(texte_voisin,lettres_a_permuter[2])
        
     } else if (str_sub(texte,i,i) == lettres_a_permuter[2]) {
        texte_decrypte = str_c(texte_voisin, lettres_a_permuter[1])
        
     } else {
        texte_decrypte = str_c(texte_voisin,str_sub(texte,i,i))
    }
  }
  return(texte_voisin)
}
...
choix_texte_voisin = function(qualite_texte_crypte, texte_crypte, alphabets_matrice_selection_ordonnees){ #changer le nom de la fonction
  
  if (qualite_texte_crypte!= 0) {
  ...
  } else if (qualite_texte_crypte == 0) {
    lettres_a_permuter = permutation_des_lettres_par_unifome(alphabets_matrice_selection_ordonnees)
    texte_decrypte = decryptage_texte_par_permutation_2_lettres(texte_crypte,lettres_a_permuter)
    return(texte_crypte)
    
  } else {
    print("erreur in choix_texte_voisin")
  }
}
...
calcul_qualite_multiplication = function(texte, matrice_de_selection) {
  qualite = 1
  facteur = 4.055*log(str_length(texte))
  
  if(is.matrix(matrice_de_selection)) {
    for(compteur in 1:(str_length(texte)-1) ) {
      qualite = qualite*matrice_de_selection[str_sub(texte,compteur,compteur) , str_sub(texte,compteur+1,compteur+1)]*facteur
    }
      
  } else {
      
    for(compteur in 1:(str_length(texte)-2) ) {
      qualite = qualite*matrice_de_selection[str_sub(texte,compteur,compteur) , str_sub(texte, compteur+1, compteur+1), str_sub(texte, compteur+2, compteur+2)]*facteur
    }
  }
...
metropolis = function(texte_crypte,texte_decrypte,qualite_texte_crypte,qualite_texte_decrypte) {
   if (qualite_texte_decrypte >= qualite_texte_crypte) {
        return(texte_decrypte)
     } else if (qualite_texte_crypte > 0 && runif(1) < qualite_texte_decrypte/qualite_texte_crypte) { 
        return(texte_decrypte)
     } else if (qualite_texte_crypte < 0 && runif(1) < exp(qualite_texte_decrypte - qualite_texte_crypte) ) {
        return(texte_decrypte)
     } else {
        return(texte_crypte)
   }
...
ecriture_alphabets_ordonnee_anglais = function() {
  data = read.table("EN-Letters.txt", header = FALSE, sep = '\t', stringsAsFactors = FALSE, na.strings = "", fill = TRUE, skip = 1)[,c(8,2)] #choisi les colonnes 8 et 2 du fichier 
  data = data[grepl("([A-Z]|\\s){1,}", data[,1], perl = TRUE),] #Ne prends que les lettres entre A et Z de l'alphabets et l'espace
  ...
  alphabets_ordonnee_anglais = unlist(data[][2])
  names(alphabets_ordonnee_anglais) = unlist(data[][1])
  return(alphabets_ordonnee_anglais)
}
...
alphabets_matrice_selection_ordonnees = ordonner_alphabets_par_frequence(corpus,alphabets_corpus)


###Remplissage de matrice de selection de valeures connues###

##Anglais : bigrammes sans espace
...
#alphabets_matrice_selection_ordonnees = ecriture_alphabets_ordonnee_anglais()
  
##Anglais : bigrammes avec espace
...
#data = data[grepl("([A-Z]|\\s){2,}", data[,1], perl = TRUE),] #Ne prends que les lettres entre A et Z de l'alphabets et l'espace
#names(data) = c("syllabe","valeur")
#row.names(data) <- NULL  #reordonne les indices de la list apres supression de certaines lignes  
#data[][1] = lapply(data[][1], tolower) #mets les lettres des lignes et colonnes en minuscule
#matrice_de_selection = remplissage_valeur_bigrames(data)
...
##Anglais : trigrammes avec espace
...
texte_a_crypter = str_sub(corpus,200,700)
texte_crypte = cryptage(texte_a_crypter, names(alphabets_matrice_selection_ordonnees))
```
...
nombre_iterations_max = 500000 #parametre
calcul_qualite = calcul_qualite_log #paramatre

texte_crypte_initiale = texte_crypte
qualite_texte_crypte_initiale = calcul_qualite(texte_crypte, matrice_de_selection)
meilleurs_resultats = list("",-Inf)
for(iteration in 1:nombre_iterations_max){
  
  qualite_texte_crypte = calcul_qualite(texte_crypte, matrice_de_selection)
  
  texte_decrypte = selection_methode_decryptage(qualite_texte_crypte, texte_crypte, alphabets_matrice_selection_ordonnees)
  
  qualite_texte_decrypte = calcul_qualite(texte_decrypte, matrice_de_selection)
  
  texte_crypte = metropolis(texte_crypte, texte_decrypte, qualite_texte_crypte, qualite_texte_decrypte)
  
  qualite_texte_decrypte = calcul_qualite(texte_crypte, matrice_de_selection)
  
  if(qualite_texte_decrypte > meilleurs_resultats[[2]]){
    meilleurs_resultats = list(texte_crypte,qualite_texte_decrypte)
  }
  
  if( iteration %% 100 == 1 ){
    cat("n = ", iteration ,"\n\n")
    cat("Original : ", qualite_texte_crypte_initiale, "\n", texte_a_crypter ,"\n\n")
    #cat("Crypte   : ","\n",texte_crypte_initial, "\n\n")
    cat("Courant  : ",qualite_texte_decrypte,"\n", texte_crypte,"\n\n")
    cat("meilleur : ",meilleurs_resultats[[2]],"\n", meilleurs_resultats[[1]], "\n","\n\n\n")
    
    if( qualite_texte_decrypte > qualite_texte_crypte_initiale ){
      print("le texte n'est pas un minimum global \n")
    }
  }
}
