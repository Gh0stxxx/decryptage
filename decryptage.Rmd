...
ordonner_alphabets_par_frequence = function(texte, alphabets){
  ...
  alphabets_ordonnees = c(numeric(length(alphabets)))

    for( i in 1:str_length(texte) ){
      for( j in 1:length(alphabets) ){
        if(str_sub(texte,i,i) == alphabets[j]) {
          alphabets_ordonnees[j] = alphabets_ordonnees[j] + 1
        }
      }
    }
...
decryptage_par_frequence_alphabets = function(texte_crypte, alphabets_matrice_selection_ordonnees,alphabets_texte_crypte_ordonnees){
  #alphabets_matrice_selection_ordonnees = names(alphabets_matrice_selection_ordonnees)
  #alphabets_texte_crypte_ordonnees = names(alphabets_texte_crypte_ordonnees)
