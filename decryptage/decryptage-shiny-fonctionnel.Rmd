...
ui <- fluidPage(
  ...
  inputPanel(
        textAreaInput("texte_a_crypter", "Texte à crypter","Enter your text here : you know caius marcius is chief enemy to the people. all: we know't, we know't. first citizen: let us kill him, and we'll have corn at our own price. is't a verdict? all: no more talking on't let it be done: away, away! second citizen: one word good citizens.",width = 750, height = 200),
        actionButton("go_cryptage", "Crypter !"),
        uiOutput("clip")
    ),
   tabsetPanel(type = "tabs",
                tabPanel("texte crypté", verbatimTextOutput("texte_crypte"))
   ),
  inputPanel(
        textAreaInput("texte_a_decrypter", "Texte à decrypter","'yv'mqfibmqv'uvq,'m'qxqfibqjyiaqpksbtq!kmpsbtqstqp,s'$q'y'!fqviqv,'qn'in&'lqk&&xqa'qjyiahv;qa'qjyiahvlq$smtvqpsvs3'yxq&'vqbtqjs&&q,s!;qkygqa'h&&q,kd'qpimyqkvqibmqiayqnmsp'lqsthvqkqd'mgspvwqk&&xqyiq!im'qvk&jsy:qiyhvq&'vqsvq-'qgiy'xqkakf;qkakf qt'piygqpsvs3'yxqiy'qaimgq:iigqpsvs3'ytl",width = 750, height = 200),
        
        actionButton("go_decryptage", "Dérypter !"),
         sliderInput("n_iterations_metropolis", "Nombre d'iterations metropolis", min=10, max=10000, value=100, step= 500)
        
    ),
    
    tabsetPanel(type = "tabs",
                tabPanel("texte decrypté", verbatimTextOutput("texte_decrypte")),
                tabPanel("texte courant", verbatimTextOutput("texte_courant"))
...
output$texte_courant <- renderText({
  paste("n= ",l," texte courant : ","   \n\n", texte_crypte)
